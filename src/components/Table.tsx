...
export default function AdminTable() {
  ...
  const getRequests = async () => {
    try {
      let response = await fetch("https://fintrackbteam.herokuapp.com/userRequest", {
        method: "GET",
        headers: {
  ...
  const updateStatus = async () => {
    if (request.status) {
      const apiUrl = `https://fintrackbteam.herokuapp.com/updaterequest/${value._id}`;
      await fetch(apiUrl, {
        mode: "cors",
        method: "PATCH",
  ...
  const deleteStatus = async () => {
    const apiUrl = `https://fintrackbteam.herokuapp.com/closerequest/${value._id}`;
    await fetch(apiUrl, {
      mode: "cors",
      method: "DELETE",
      headers: {
  ...
  const submitRequest = async (e?: any) => {
    // e.preventDefault();
    // console.log(micro);
    const apiUrl = `https://fintrackbteam.herokuapp.com/updaterequest/${editID}`;
    await fetch(apiUrl, {
      mode: "cors",
      method: "PATCH",
    ...
    <>
      <Collapse in={openDelete}>
        <Alert onClick={() => deleteStatus()}>
          confirm delete!
          <CheckIcon style={{ marginLeft: "600px" }} />
        </Alert>
      </Collapse>
